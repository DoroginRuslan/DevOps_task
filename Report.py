...
class Report(object):
    ...
    def initFolderReportFile(self):
        try:
            open(self.report_file_path, "w").close()
        except Exception:
            print("Ошибка создания файла '", self.report_file_path)

    # Метод добавляет запись в файл отчёта папки
    def addToFolderReportFile(self, data):
    ...
    # Метод для чтения файла отчёта папки
    def readFolderReportFile(self):
        file_report = open(self.report_file_path, "r")
        try:
            result = file_report.read()
            file_report.close()
            return result
        except Exception:
            file_report.close()
    ...
    def reportErrorTest_1(self, errFolder):
        self.addToFolderReportFile("directory missing: " + errFolder)
    ...
    def reportErrorTest_2_missing(self, errFolder, pairFolder, noneFiles):
        ...
        for lostFile in noneFiles:
            ...
            reportText += " '" + lostFile + "'"
            trigger_comma = True
        self.addToFolderReportFile(reportText)
    ...
    def reportErrorTest_3_Error(self, filePath, lineNumber, line):
        self.addToFolderReportFile(filePath + "(" + str(lineNumber) + "): " + line)

    # Выводит в файл отчёта ошибку теста 3 (потеряно 'Solver finished at')
    def reportErrorTest_3_miss_solver(self, filePath):
        self.addToFolderReportFile(filePath + ": missing 'Solver finished at'")
    ...
    def reportErrorTest_4_memory_peak(self, filePath, mp_ft_run, mp_ft_reference):
        self.addToFolderReportFile("{}: different 'Total' of bricks"
                                   " (ft_run={}"
                                   ", ft_reference={}"
                                   ", rel.diff={:.2f}"
                                   ", criterion=4)".format(filePath, str(mp_ft_run), str(mp_ft_reference),
                                                           calcRelDiff(mp_ft_run, mp_ft_reference)))
    ...
    def reportErrorTest_4_MESH_Bricks(self, filePath, mp_ft_run, mp_ft_reference):
        self.addToFolderReportFile("{}: different 'Total' of bricks"
                                   ...
                                   ", ft_reference={:g}"
                                   ", rel.diff={:.2f}"
                                   ", criterion=0.1)".format(filePath, mp_ft_run, mp_ft_reference,
                                                             calcRelDiff(mp_ft_run, mp_ft_reference)))
